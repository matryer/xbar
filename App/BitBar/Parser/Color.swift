...
final class Color: Param, Equatable  {
  ...
  var values: [String: Any] {
    return ["raw": raw, "color": color ?? "unknown"]
  }
  ...
  func menu(didLoad menu: Menuable) {
    guard let aColor = color else {
      return menu.add(error: "Could not parse color from \(raw)")
    }

    menu.set(color: aColor)
  }

  func equals(_ param: Param) -> Bool {
    if let color = param as? Color {
      return color == self
    }
  ...
  static func == (lhs: Color, rhs: Color) -> Bool {
    return lhs.raw == rhs.raw && lhs.color == rhs.color
  }
}
