...
## Plugin API
  ...
  * Your lines might contain `|` to separate the title from other parameters, such as...
    ...
    * 'image=..' set an image for this item. The image data must be passed as base64 encoded string. The imageformat can be any of the formats supported by Mac OS X (Note: images will only be displayed on OS X 10.9 or later)
