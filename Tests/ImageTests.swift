...
//          expect($0.value).to(equal("dGVzdGluZw=="))
//        }
//      }
//
//      context("whitespace") {
...
//        verifyBase64(Pro.getTemplateImage(), "templateImage")
    ...
    describe("Base64Image") {
      ...
      it("should handle base64 image tag") {
        self.match(Pro.getImage(), "image=" + image) { image in
          expect(image).toNot(beNil())
          expect(image.key).to(equal("image"))
        }
      }

      it("should handle base64 imageTemplate tag") {
        self.match(Pro.getTemplateImage(), "templateImage=" + image) { image in
          expect(image).toNot(beNil())
          expect(image.key).to(equal("templateImage"))
        }
      }
    }
  }
